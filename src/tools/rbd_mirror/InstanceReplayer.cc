...
void InstanceReplayer<I>::acquire_image(InstanceWatcher<I> *instance_watcher,
  ...
  if (it == m_image_replayers.end()) {
    ...
    start_image_replayer(image_replayer, on_finish);
  } else {
    ...
    image_replayer->restart(new C_TrackedOp(m_async_op_tracker, on_finish));
  }
}
...
void InstanceReplayer<I>::start_image_replayer(
  ...
  dout(10) << "global_image_id=" << global_image_id << dendl;
  image_replayer->start(new C_TrackedOp(m_async_op_tracker, on_finish), false);
}
