...
void BootstrapRequest<I>::handle_prepare_local_image(int r) {
  ...
  if (state_builder != nullptr && state_builder->is_local_primary()) {
    ...
    // force locale image name update
    finish(-ENOMSG);
    return;
  }
...
void BootstrapRequest<I>::prepare_remote_image() {
  ...
  if (m_it_peer == m_peers.end()) {
    ...
    if (state_builder->remote_image_id.empty() && (state_builder->local_image_id.empty() || state_builder->is_linked())) {
  ...
  ceph_assert(m_pool_meta_cache->get_remote_pool_meta(m_it_peer->io_ctx.get_id(),
    m_it_peer->uuid, &remote_pool_meta) == 0);
