...
PoolReplayer<I>::PoolReplayer(
  ...
  m_peers(peers),
  m_args(args),
  m_lock(ceph::make_mutex("rbd::mirror::PoolReplayer " + local_pool_id)),
...
int PoolReplayer<I>::update_peers(const Peers& peers)
{
  ...
  for (auto kv_it = m_remote_peers_ressource.begin();
    ...
    auto peer_it = peers.find(kv_it->first);
    if (peer_it != peers.end()) {
      continue;
    }

    dout(10) << "remove_peer: peer_spec=" << kv_it->first << dendl;
  ...
  for (const auto& peer_spec : peers) {
...
int PoolReplayer<I>::add_peer(const PeerSpec& peer_spec, C_Gather* gather_ctx) {
  ...
  ceph_assert(!peer->remote_pool_meta.mirror_uuid.empty());
  m_pool_meta_cache->set_remote_pool_meta(peer->io_ctx.get_id(),
    peer_spec.uuid, peer->remote_pool_meta);
...
void PoolReplayer<I>::shut_down() {
  ...
  for (auto& kv : m_remote_peers_ressource) {
    if (kv.second.remote_pool_poller) {
      C_SaferCond ctx;
      kv.second.remote_pool_poller->shut_down(&ctx);
      ctx.wait();

      m_pool_meta_cache->remove_remote_pool_meta(
          kv.second.peer.io_ctx.get_id(), kv.first.uuid);
    }
  }
...
void PoolReplayer<I>::handle_remote_pool_meta_updated(
  ...
  if (!m_default_namespace_replayer) {
    auto peer_it = m_remote_peers_ressource.find(peer);
    ceph_assert(peer_it != m_remote_peers_ressource.end());
    peer_it->second.peer.remote_pool_meta = remote_pool_meta;
    return;
  }
